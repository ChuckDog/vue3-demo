...
const mutations = {
  ...
  markAsAllDone(state: State, allCompleted: boolean) {
    state.todoList.forEach((item) => (item.completed = allCompleted));
  },
};

const getters = {
  allDone(state: State) {
    return !state.todoList.filter((todo: Todo) => todo.completed === false)
      .length;
  },
  ...
  filteredTodoList(state: State) {
    return {
      all: () => state.todoList,
      active: () =>
        state.todoList.filter((todo: Todo) => todo.completed === false),
      completed: () =>
        state.todoList.filter((todo: Todo) => todo.completed === true),
    };
  },
};
