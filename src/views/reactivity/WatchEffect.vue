<template>
  ...
  <ul style="text-align: left; padding-left: 300px">
    <li>
      1.在响应式地跟踪其依赖项时立即运行一个函数，并在更改依赖项时重新运行它。
    </li>
    <li>
      2.使用 watchEffect() 函数定义副作用函数。默认值时会首先执行一次副作用函数,
      只在监听对象最后一次变更后触发副作用函数。
    </li>
    <li>3.返回值是停止函数,一旦调用后就不再监听该对象。</li>
    <li>
      4.watchEffect的默认的更新时机是异步的。执行时机在nextTick（Promise.resolve()）之后。。
    </li>
    <li>
      5.有时副作用函数会执行一些异步的副作用，这些响应需要在其失效时清除。接收一个
      onInvalidate 函数作入参，用来注册清理失效时的回调。
    </li>
    <li>
      6. &nbsp;&nbsp; <br />
      &nbsp;&nbsp; a、effect() 函数来自于 @vue/reactivity ，而 watchEffect()
      函数来自于 @vue/runtime-core。<br />
      &nbsp;&nbsp; b、它们的区别在于：effect() 是非常底层的实现，watchEffect()
      是基于 effect() 的封装，<br />
      &nbsp;&nbsp; c、watchEffect()
      会维护与组件实例以及组件状态(是否被卸载等)的关系，如果一个组件被卸载，那么
      watchEffect() 也将被 stop，但 effect() 则不会。
    </li>
  </ul>
</template>
