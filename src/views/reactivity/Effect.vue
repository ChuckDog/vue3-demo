<template>
  ...
  <ul style="text-align: left; padding-left: 300px">
    ...
    <li>
      2.用来运行副作用函数，默认是立即执行的。它可以是 lazy
      的，这时我们可以手动执行它。
    </li>
    <li>
      3.@vue/reactivity 提供了 stop 函数用来停止一个副作用。effect
      函数会返回effect 本身，把这个返回值传递给 stop 函数，就可以停止掉这个
      effect。
    </li>
    <li>
      4.effect
      传递第二个参数作为选项，可以指定“调度器”。所谓调度器就是用来指定如何运行副作用函数的。实际上就是
      watchEffect() 函数的实现思路。
    </li>
  </ul>
</template>
