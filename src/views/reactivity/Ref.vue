<template>
  ...
  <ul style="text-align: left; padding-left: 300px">
    <li>
      1.接受一个参数值并返回一个响应式且可改变的 ref 对象。ref
      对象拥有一个指向内部值的单一属性 .value。
    </li>
    <li>
      2.ref 是 reactive 的一个变形版本，reactive 只接受对象作为入参，ref
      解决值类型的数据响应，如果传入 ref 的是一个对象，内部会调 reactive。
    </li>
    <li>3.isRef 用来判断某个值是否为 ref() 创建出来的对象。</li>
    <li>4.unref 返回 ref 代理的原始对象，这是一个转义接口。</li>
    <li>
      5.shallowRef 只代理 ref 对象本身，也就是说只有 .value 是被代理的，而
      .value 所引用的对象并没有被代理。
    </li>
    <li>6.toRef() 函数用来把一个响应式对象的的某个 key 值转换成 ref。</li>
    <li>7.toRefs() 函数直接把一个响应式对象的所有key都转成 ref。</li>
  </ul>
</template>
...
export default {
  setup() {
    ...
    setTimeout(() => {
      ...
      watchEffect(() => {
        console.log(
          "监听shallowRef对象 shallowState.value:" + shallowRefState.value
        );
      });
    ...
    setTimeout(() => {
      ...
      watchEffect(() => {
        console.log(
          "监听shallowRef对象 shallowState2.value.foo:" +
