<template>
  ...
  <ul style="text-align: left; padding-left: 300px">
    <li>
      1.与watchEffect相比，惰性地执行副作用。更具体地说明应触发侦听器重新运行的状态。访问侦听状态的先前值和当前值。
    </li>
    <li>
      2.使用 watch()
      函数可以监听多个数据源。源数组中必须是ref对象或返回reactive对象的函数。
    </li>
    <li>
      3.watch的默认的更新时机是异步的。执行时机在nextTick（Promise.resolve()）之后。
    </li>
...
export default {
  setup() {
    ...
    watch(
      [count, () => obj.count],
      ([count, objCount], [prevCount, prevObjCount]) => {
        console.log(
          "监听数组中的ref对象count:" + prevCount,
          count,
          ",监听数组中的reactive对象obj.count",
          prevObjCount,
          objCount
        );
      }
